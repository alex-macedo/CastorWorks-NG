...
import { cn, getSingleRenderableChild } from "@/lib/utils";
...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, width: widthProp, onWidthChange: setWidthProp, className, style, children, ...props }, ref) => {
  ...
  const [_width, _setWidth] = React.useState(256); // Default 16rem in pixels
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        data-sidebar="rail"
        aria-label={t("ariaLabels.resizeSidebar")}
...
const SidebarGroupLabel = React.forwardRef<HTMLDivElement, React.ComponentProps<"div"> & { asChild?: boolean }>(
  ({ className, asChild = false, children, ...props }, ref) => {
    const slotChild = getSingleRenderableChild(children);
    const shouldUseSlot = asChild && React.isValidElement(slotChild);
    const Comp = shouldUseSlot ? Slot : "div";
    ...
    return (
      <Comp
        ...
        {...props}
      >
        {shouldUseSlot ? slotChild : children}
      </Comp>
    );
  },
);
...
>(({ asChild = false, isActive = false, variant = "default", size = "default", tooltip, className, children, ...props }, ref) => {
  ...
  const shouldUseSlot = asChild && React.isValidElement(slotChild);
  const Comp = shouldUseSlot ? Slot : "button";
  const buttonChildren = shouldUseSlot ? slotChild : children;
