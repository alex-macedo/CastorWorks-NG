-- ============================================================================
-- CastorWorks Architect Projects Data Cleanup - RPC Function
-- Created: 2025-01-30
...
CREATE OR REPLACE FUNCTION delete_architect_projects_safely(
    p_target_projects TEXT[] DEFAULT ARRAY['project-01', 'project-02'],
...
BEGIN
    ...
    IF p_target_projects IS NULL OR array_length(p_target_projects, 1) = 0 THEN
        RAISE EXCEPTION 'Target projects array cannot be empty';
    END IF;
    ...
    IF p_create_backup THEN
        ...
        EXECUTE format('CREATE TEMPORARY TABLE backup_architect_projects_%s AS 
            ...
            UNION ALL
            SELECT ''projects'', id::text, name, client_id::text, created_at
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_task_comments', 
            (SELECT COUNT(*) FROM architect_task_comments WHERE task_id IN (
                SELECT id FROM architect_tasks WHERE project_id = ANY(p_target_projects)
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_tasks', 
            (SELECT COUNT(*) FROM architect_tasks WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete architect tasks';
    ELSE
        DELETE FROM architect_tasks WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_meetings', 
            (SELECT COUNT(*) FROM architect_meetings WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete architect meetings';
    ELSE
        DELETE FROM architect_meetings WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_briefings', 
            (SELECT COUNT(*) FROM architect_briefings WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete architect briefings';
    ELSE
        DELETE FROM architect_briefings WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_moodboard_images', 
            (SELECT COUNT(*) FROM architect_moodboard_images WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete moodboard images';
    ELSE
        DELETE FROM architect_moodboard_images WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_moodboard_colors', 
            (SELECT COUNT(*) FROM architect_moodboard_colors WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete moodboard colors';
    ELSE
        DELETE FROM architect_moodboard_colors WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_moodboard_sections', 
            (SELECT COUNT(*) FROM architect_moodboard_sections WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete moodboard sections';
    ELSE
        DELETE FROM architect_moodboard_sections WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_site_diary', 
            (SELECT COUNT(*) FROM architect_site_diary WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete site diary entries';
    ELSE
        DELETE FROM architect_site_diary WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'architect_client_portal_tokens', 
            (SELECT COUNT(*) FROM architect_client_portal_tokens WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete client portal tokens';
    ELSE
        DELETE FROM architect_client_portal_tokens WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_photos', 
            (SELECT COUNT(*) FROM project_photos WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project photos';
    ELSE
        DELETE FROM project_photos WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_documents', 
            (SELECT COUNT(*) FROM project_documents WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project documents';
    ELSE
        DELETE FROM project_documents WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_budget_items', 
            (SELECT COUNT(*) FROM project_budget_items WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project budget items';
    ELSE
        DELETE FROM project_budget_items WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_resources', 
            (SELECT COUNT(*) FROM project_resources WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project resources';
    ELSE
        DELETE FROM project_resources WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_materials', 
            (SELECT COUNT(*) FROM project_materials WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project materials';
    ELSE
        DELETE FROM project_materials WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_activities', 
            (SELECT COUNT(*) FROM project_activities WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project activities';
    ELSE
        DELETE FROM project_activities WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_phases', 
            (SELECT COUNT(*) FROM project_phases WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project phases';
    ELSE
        DELETE FROM project_phases WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_milestones', 
            (SELECT COUNT(*) FROM project_milestones WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project milestones';
    ELSE
        DELETE FROM project_milestones WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_team_members', 
            (SELECT COUNT(*) FROM project_team_members WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete project team members';
    ELSE
        DELETE FROM project_team_members WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'time_logs', 
            (SELECT COUNT(*) FROM time_logs WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete time logs';
    ELSE
        DELETE FROM time_logs WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'daily_logs', 
            (SELECT COUNT(*) FROM daily_logs WHERE project_id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete daily logs';
    ELSE
        DELETE FROM daily_logs WHERE project_id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'projects', 
            (SELECT COUNT(*) FROM projects WHERE id = ANY(p_target_projects)), 
            'DRY_RUN', 'Would delete main project records';
    ELSE
        DELETE FROM projects WHERE id = ANY(p_target_projects);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
    ...
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'seed_data_registry', 
            (SELECT COUNT(*) FROM seed_data_registry 
             WHERE entity_id = ANY(p_target_projects)
    ...
    RETURN QUERY SELECT v_step_counter, 'summary', 
        ...
        format('Process completed in % seconds. Mode: %', 
...
BEGIN
    ...
    WHERE project_id = ANY(p_target_projects)
    ...
    SELECT 'projects', 
           ...
           CASE WHEN COUNT(*) = 0 THEN 'CLEAN' ELSE 'RECORDS_REMAINING' END
    FROM projects 
    WHERE id = ANY(p_target_projects)
