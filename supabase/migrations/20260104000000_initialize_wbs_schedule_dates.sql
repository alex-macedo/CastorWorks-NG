-- Initialize WBS Schedule Dates
-- Purpose: Calculate and set start/end dates for WBS items based on project start date and durations
-- Uses the linked project_phases table for schedule data storage

-- First, add standard_duration_days to project_wbs_items if not exists
ALTER TABLE public.project_wbs_items 
  ADD COLUMN IF NOT EXISTS standard_duration_days INT;

-- Add constraint for non-negative duration
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'project_wbs_items_standard_duration_days_nonneg'
  ) THEN
    ALTER TABLE public.project_wbs_items
      ADD CONSTRAINT project_wbs_items_standard_duration_days_nonneg
      CHECK (standard_duration_days IS NULL OR standard_duration_days >= 0);
  END IF;
END $$;

-- Create function to initialize WBS schedule dates
...
BEGIN
  ...
  INSERT INTO temp_wbs_dates (wbs_item_id, parent_id, level, sort_order, duration_days, code_path)
  SELECT id, parent_id, level, sort_order, COALESCE(standard_duration_days, 1), code_path
  FROM wbs_tree
  ...
  FOR v_item IN (
    ...
    SELECT COALESCE(SUM(duration_days), 0) INTO v_child_duration
    FROM temp_wbs_dates 
    ...
    WHERE wbs_item_id = v_item.wbs_item_id
    RETURNING end_date INTO v_parent_end_date;
    ...
    BEGIN
      FOR v_child IN (
        ...
        SET start_date = v_child_start,
            end_date = v_child_start + COALESCE(duration_days, 1) - 1
        WHERE wbs_item_id = v_child.wbs_item_id;
  ...
  -- Update project_phases with calculated dates for phase-type WBS items
  UPDATE public.project_phases pp
  SET 
    start_date = twd.start_date,
...
Updates the linked project_phases records with the calculated schedule.
...
GRANT EXECUTE ON FUNCTION public.initialize_wbs_schedule_dates(UUID) TO authenticated;
