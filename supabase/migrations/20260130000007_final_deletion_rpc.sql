-- ============================================================================
-- Final deletion RPC function for architect projects (with task statuses)
-- Created: 2025-01-30
-- Description: Complete deletion including project_task_statuses
-- ============================================================================

-- Drop and recreate the function with task statuses
...
BEGIN
    ...
    IF p_create_backup THEN
        RETURN QUERY SELECT 1, 'backup_creation', 0::BIGINT, 'SUCCESS', 
            'Backup creation skipped for simplicity';
    END IF;
    ...
    -- Step 3: Delete project task statuses (NEW)
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'project_task_statuses', 
            (SELECT COUNT(*) FROM project_task_statuses WHERE project_id = ANY(v_target_project_ids))::BIGINT, 
            'DRY_RUN', 'Would delete project task statuses';
    ELSE
        DELETE FROM project_task_statuses WHERE project_id = ANY(v_target_project_ids);
        GET DIAGNOSTICS v_total_deleted = ROW_COUNT;
        RETURN QUERY SELECT v_step_counter, 'project_task_statuses', v_total_deleted, 'SUCCESS', 
            'Deleted ' || v_total_deleted || ' project task status records';
    END IF;

    -- Step 4: Delete architect meetings
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 5: Delete architect briefings
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 6: Delete architect opportunities
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 7: Delete moodboard images (children of sections)
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 8: Delete moodboard colors
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 9: Delete moodboard sections
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 10: Delete architect site diary entries
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 11: Delete architect client portal tokens
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 13: Delete project documents
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 14: Delete project budget items
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 15: Delete project resources
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 16: Delete project materials
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 17: Delete project activities
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 19: Delete project milestones
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 20: Delete project team members
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 23: Delete the main project records (LAST)
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
    ...
    -- Step 24: Clean up seed registry entries
    v_step_counter := v_step_counter + 1;
    IF p_dry_run THEN
        RETURN QUERY SELECT v_step_counter, 'seed_data_registry', 
            0::BIGINT, 'DRY_RUN', 'Would delete seed registry entries';
    ELSE
        -- Skip seed registry cleanup for now
        RETURN QUERY SELECT v_step_counter, 'seed_data_registry', 0::BIGINT, 'SKIPPED', 
            'Seed registry cleanup skipped';
    END IF;
