-- Migration: Fix populate_budget_from_cost_control_template to reuse existing baseline version
-- Purpose: Check for existing baseline version instead of always creating new one

BEGIN;

-- Drop and recreate function with baseline version reuse logic
...
BEGIN
  ...
  -- Check if a baseline budget version already exists for this project
  ...
  -- If no baseline version exists, create one
...
Reuses existing baseline budget version if one exists, otherwise creates new baseline.
