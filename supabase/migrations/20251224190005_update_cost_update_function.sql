-- Update update_budget_items_with_sinapi_costs function to use new table
-- Migration: 20251224190005_update_cost_update_function.sql
-- Purpose: Update function to use sinapi_items instead of sinapi_catalog
...
BEGIN
  ...
  LOOP
    ...
    IF sinapi_item_data IS NOT NULL THEN
      UPDATE public.budget_line_items
      SET
        unit_cost_material = COALESCE(sinapi_item_data.sinapi_material_cost, 0),
        unit_cost_labor = COALESCE(sinapi_item_data.sinapi_labor_cost, 0),
        updated_at = NOW()
...
COMMENT ON FUNCTION public.update_budget_items_with_sinapi_costs IS 
  'Updates existing budget line items with unit costs from sinapi_items catalog. Useful for backfilling costs for items that were created before SINAPI lookup was working correctly.';
