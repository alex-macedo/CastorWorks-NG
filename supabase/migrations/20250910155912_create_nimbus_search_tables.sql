...
create table if not exists "public"."page_nimbus" (
...
drop policy if exists "authenticated can read page_nimbus"
on public.page_nimbus;

-- Allow authenticated users to read nimbus documentation pages (public reference data)
drop policy if exists "service_select_page_nimbus"
on public.page_nimbus;

create policy "service_select_page_nimbus"
on public.page_nimbus
for select
to authenticated
using ((auth.jwt() ->> 'role') = 'service_role');

create table if not exists "public"."page_section_nimbus" (
  id bigint primary key generated always as identity,
...
DROP POLICY IF EXISTS "anon can read page_section_nimbus"
  ON public.page_section_nimbus;

drop policy if exists "authenticated can read page_section_nimbus"
on public.page_section_nimbus;

-- Allow authenticated users to read nimbus documentation sections (public reference data)
drop policy if exists "service_select_page_section_nimbus"
on public.page_section_nimbus;

create policy "service_select_page_section_nimbus"
on public.page_section_nimbus
for select
to authenticated
using ((auth.jwt() ->> 'role') = 'service_role');
...
create index if not exists fts_search_index_content_nimbus
on page_nimbus
using gin(fts_tokens);

create index if not exists fts_search_index_title_nimbus
on page_nimbus
