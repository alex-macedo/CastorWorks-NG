...
-- 1. Reassign "next_up" and "in_progress" items to the next open sprint, or to Backlog (sprint_id = NULL) if none.
-- 2. Keep existing snapshot + template release notes; AI generation is triggered by the client via Edge Function.
...
BEGIN
  ...
  -- Reassign "next_up" and "in_progress" to next open sprint or Backlog (sprint_id = NULL)
  SELECT id INTO v_next_open_sprint_id
  FROM sprints
  WHERE status = 'open' AND id != p_sprint_id
  ORDER BY created_at DESC
  LIMIT 1;

  UPDATE roadmap_items
  SET sprint_id = v_next_open_sprint_id
  WHERE sprint_id = p_sprint_id
    AND status IN ('next_up', 'in_progress');
