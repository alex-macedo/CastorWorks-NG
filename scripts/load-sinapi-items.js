 ...
 * 
 * This script reads the sinapi-list-project.csv file and loads
...
async function loadSinapiItemsData() {
  try {
    ...
    // Read CSV file
    const csvPath = path.join(__dirname, '../docs/sinapi-list-project.csv');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const [item, sinapiCode, description, unit, quantity] = columns;
      
      // Skip header rows or empty data
      if (!sinapiCode || sinapiCode === 'Cód. SINAPI' || sinapiCode === '') {
        if (description && description !== 'Descrição') {
          // This might be a category header, still include it
          const record = {
            ...
            sinapi_material_cost: 0,
            sinapi_labor_cost: 0,
            sinapi_type: 'Materials', // Default category
      ...
      // Determine if it's labor or materials based on code or description
      const sinapiType = determineSinapiType(sinapiCode, description);
      ...
      const record = {
        ...
        sinapi_unit: unit || '',
        sinapi_quantity: parseFloat(quantity) || 0,
        sinapi_material_cost: 0, // Will be updated later with actual costs
        sinapi_labor_cost: 0,    // Will be updated later with actual costs
...
function determineSinapiType(sinapiCode, description) {
  if (!description) return 'Materials';
  
  // Labor keywords in Portuguese
  const laborKeywords = [
    'ENGENHEIRO', 'MESTRE', 'CARPINTEIRO', 'SERVIÇO', 'SERVICO',
    'MÃO DE OBRA', 'MAO DE OBRA', 'CHP', 'MES', 'H', 'HORISTA',
    'DIARIA', 'ENCARGOS', 'COMPLEMENTARES', 'OBRA'
  ];
  
  const descriptionUpper = description.toUpperCase();
  
  // Check if description contains labor keywords
  ...
  // Check if SINAPI code suggests labor (based on common patterns)
  if (sinapiCode) {
    const code = sinapiCode.toString();
    // Labor codes often start with 9, 8, or are in specific ranges
    if (code.startsWith('9') || code.startsWith('8') || code.startsWith('40')) {
      return 'Labor';
    }
  }
