...
import { Plus, Search, User, Calendar, TrendingUp, LayoutGrid, List, Table as TableIcon, ArrowUpDown, Folder, Pencil, Sparkles, Briefcase } from "lucide-react";
...
import { createProjectColumns } from "@/components/Projects/project-columns";
...
export default function ArchitectProjectsListPage() {
  ...
  const isArchitect = roles?.some(r => r.role === 'architect');
  ...
  const canEditProject = (project: any) => {
    ...
    // Architects can edit projects they have access to (they can see them in this list)
  ...
  const handleGenerateBudget = async (project?: any) => {
    ...
    try {
      ...
      navigate(`/architect/projects/${projectToUse.id}`);
  ...
  const handleSkipBudget = () => {
    if (pendingProject) {
      setBudgetPromptOpen(false);
      navigate(`/architect/projects/${pendingProject.id}`);
      setPendingProject(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <SidebarHeaderShell variant={roles?.some(r => r.role === 'architect') ? 'architect' : 'default'}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <h1 className="text-3xl font-bold">{t("architect.projects.title")}</h1>
            <p className="text-white/90 text-sm">{t("architect.portfolio.sections.projectsSubtitle")}</p>
          </div>
          {canCreateProject && (
             <Button
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Folder} title={t('common.noData')} description={t('projects:emptyStateDescription')} primaryAction={canCreateProject ? { label: t('architect.projects.new'), onClick: () => setNewProjectSheetOpen(true) } : undefined} />
      ) : viewMode === "table" ? (
        <DataTable columns={createProjectColumns({ t, currency, onEdit: handleEdit, onView: (project: any) => navigate(`/architect/projects/${project.id}`), getProjectTypeLabel, isDemoProject: (id: string) => seedIds?.has(id) ?? false, enableRowSelection: false, canEditProject }) as any} data={filteredProjects as any[]} enableRowSelection={false} enableColumnVisibility={true} enableFiltering={false} pageSize={25} onRowClick={(project: any) => navigate(`/architect/projects/${project.id}`)} />
      ) : viewMode === "cards" ? (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {(filteredProjects as any[]).map((project: any) => (
            <Card key={project.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              <div className="relative h-40 cursor-pointer overflow-hidden group" onClick={() => navigate(`/architect/projects/${project.id}`)}>
              ...
              <CardContent className="p-3 flex flex-col justify-between min-h-[200px]">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-medium">{project.avg_progress || 0}%</span>
                      </div>
                      <Progress value={project.avg_progress || 0} className="h-1.5" />
                    </div>
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-muted-foreground">{t('projects:budgetUsed')}</span>
                        <span className="font-medium">{project.budget_used_percentage || 0}%</span>
                      </div>
                      <Progress value={project.budget_used_percentage || 0} className="h-1.5" />
                    </div>
                  </div>
                  <div className="space-y-1">
                    {project.manager && (
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 max-w-[20%]">
                        {(project.budget_total || project.total_area != null) && (
                          <div className="flex items-center gap-1 text-xs text-muted-foreground justify-end">
                            {project.total_area != null && (
                              <div className="flex items-center gap-1">
                                <span className="font-medium">{project.total_area} m²</span>
                              </div>
                            )}
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <CompactValue
                                  compactValue={numberFormat === 'compact' 
                                    ? formatCompactCurrency(Number(project.budget_total), currency)
                                    : formatCurrency(Number(project.budget_total), currency)
                                  }
                                  fullValue={formatCurrency(Number(project.budget_total), currency)}
                ...
                <div className="flex gap-1 mt-2">
                  {canEditProject(project) && (
                    <Button variant="glass-style-dark" className="flex-1" onClick={() => handleEdit(project)}>
                      <Pencil className="mr-1 h-3 w-3" />
                      {t('projects:editProject')}
                    </Button>
                  )}
                  <Button variant="glass-style-dark" className="flex-1" onClick={() => navigate(`/architect/projects/${project.id}`)}>
            ...
            <Card key={project.id} className="hover:shadow-lg transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-center gap-6">
                  <div className="cursor-pointer overflow-hidden rounded-lg group" onClick={() => navigate(`/architect/projects/${project.id}`)}>
                  ...
                  <div className="flex-1 space-y-3">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-2">{seedIds?.has(project.id) && <Badge className="bg-blue-500 hover:bg-blue-600 text-white"><Sparkles className="h-3 w-3 mr-1" />{t('projects:demoData')}</Badge>}<ProjectScheduleStatusBadge status={getProjectScheduleStatus(project)} /></div>
                    </div>
                  ...
                  <div className="flex gap-2 self-center">{!(isClient || isViewer) && <Button variant="outline" size="sm" onClick={() => handleEdit(project)}><Pencil className="mr-2 h-4 w-4" />{t('common.edit')}</Button>}<Button size="sm" onClick={() => navigate(`/architect/projects/${project.id}`)}>{t('projects:viewDetails')}</Button></div>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Sheet open={newProjectSheetOpen} onOpenChange={setNewProjectSheetOpen}><SheetContent side="right" className="sm:max-w-[83.2rem] overflow-y-auto"><SheetHeader><SheetTitle>{t("architect.projects.new")}</SheetTitle></SheetHeader><div className="mt-6"><ProjectForm defaultValues={draftFormData} onSubmit={handleNewProjectSubmit} onCancel={() => { setNewProjectSheetOpen(false); setDraftFormData(null); }} /></div></SheetContent></Sheet>
