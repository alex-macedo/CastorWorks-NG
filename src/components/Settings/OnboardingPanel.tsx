...
import { UserCheck, Users, RefreshCw, CheckCircle } from 'lucide-react'
...
export function OnboardingPanel() {
  ...
  // Track the selected role per pending user
  ...
  const handleConfirm = async (userId: string, userEmail: string) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>{t('settings:onboarding.description')}</CardDescription>
            </div>
          </div>
          <Button
            size="sm"
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="space-y-1">
            {/* Table header */}
            <div className="grid grid-cols-[1fr_1fr_auto_auto] gap-4 items-center px-3 py-2 text-xs font-medium text-muted-foreground border-b">
              <span>{t('settings:onboarding.table.email')}</span>
              <span className="hidden sm:block">{t('settings:onboarding.table.registeredAt')}</span>
            ...
            {pendingUsers.map((user) => (
              <div
                key={user.id}
                className="grid grid-cols-[1fr_1fr_auto_auto] gap-4 items-center px-3 py-3 rounded-lg hover:bg-muted/50 transition-colors"
              >
                {/* Email + Display Name */}
                ...
                <Button
                  ...
                  disabled={confirmOnboarding.isPending}
                >
                  <UserCheck className="h-3.5 w-3.5 mr-1.5" />
                  {t('settings:onboarding.confirmButton')}
                </Button>
              </div>
            ))}
