-- Date: 2026-01-25
-- Purpose: Ensure initialize_wbs_schedule_dates updates WBS items and activities
--          so schedule dates populate across phases, activities, and WBS.
...
DECLARE
  ...
  v_wbs_items_updated INT DEFAULT 0;
...
BEGIN
  ...
  -- Update project_wbs_items with calculated dates
  UPDATE public.project_wbs_items wi
  SET start_date = twd.start_date,
      end_date = twd.end_date,
      updated_at = NOW()
  FROM temp_wbs_dates twd
  WHERE wi.id = twd.wbs_item_id
    AND wi.project_id = _project_id;

  GET DIAGNOSTICS v_wbs_items_updated = ROW_COUNT;
  RAISE NOTICE 'Updated % project_wbs_items with schedule dates', v_wbs_items_updated;
