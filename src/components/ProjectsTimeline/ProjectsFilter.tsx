...
import { Priority } from "@/types/projectsTimeline";
        ...
        <div className="py-1">
          {priorities.map((priority) => (
            <button
              ...
              onClick={() => {
                setPriorityFilter(
                  priority.value === "all" ? null : (priority.value as Priority)
                );
                setOpen(false);
              }}
              className={cn(
                "w-full text-left px-3 py-2 text-sm rounded-sm hover:bg-primary/10 flex items-center justify-between transition-colors",
                isSelected(priority.value) && "bg-primary/10"
              )}
            >
