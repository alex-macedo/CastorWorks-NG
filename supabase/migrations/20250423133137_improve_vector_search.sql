...
-- Ensure we can redefine search_content with a new return type
DO $$
BEGIN
  IF to_regprocedure(
       'public.search_content(vector,boolean,double precision,integer)'
     ) IS NOT NULL THEN
    DROP FUNCTION public.search_content(
      vector,
      boolean,
      double precision,
      integer
