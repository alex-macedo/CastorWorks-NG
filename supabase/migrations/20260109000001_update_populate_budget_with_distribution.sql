-- Migration: Update populate_budget_from_cost_control_template to distribute budget using percentages
-- Purpose: Fix bug where Cost Control budgets generate with zero amounts
-- Now uses project.budget_total and WBS phase budget_percentage to calculate actual amounts
...
-- Create updated function with budget distribution logic
...
BEGIN
  ...
  LOOP
    ...
    IF phase_id_val IS NULL THEN
      INSERT INTO public.project_phases (
        ...
        'pending',
        0,
        v_phase_budget,  -- Set the allocated budget for this phase
        'budget',
        wbs_phase.id
      ...
      IF cost_code_rec.id IS NOT NULL THEN
        ...
        IF existing_item_id IS NULL THEN
          ...
          INSERT INTO public.budget_line_items (
            ...
            v_amount_per_cost_code,  -- Set calculated amount as material cost
            0.00,                     -- Labor cost separate (can be adjusted by user)
  ...
  -- Return results
  RETURN QUERY SELECT v_phases_created, v_items_created, v_items_skipped, v_total_budget_distributed;
END;
$$;
...
'Populates budget_line_items for Cost Control budgets from project WBS structure with budget distribution.
1. Gets project budget_total from projects table
2. For each WBS phase: calculates phase budget = budget_total × budget_percentage / 100
3. Distributes phase budget equally among all cost codes in that phase
4. Sets unit_cost_material to calculated amount per cost code
5. Creates project_phases with budget_allocated set to calculated phase budget
Returns (phases_created, items_created, items_skipped, total_budget_distributed).';
