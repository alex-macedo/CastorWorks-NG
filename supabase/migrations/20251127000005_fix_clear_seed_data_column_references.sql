-- Fix column reference errors in clear_seed_data_records function
-- The column is 'request_id', not 'purchase_request_id'
...
AS $function$
BEGIN
  -- Delete quotes for purchase request items not in protected projects
  DELETE FROM quotes
  WHERE purchase_request_item_id IN (
    SELECT pri.id
    FROM purchase_request_items pri
    JOIN project_purchase_requests ppr ON ppr.id = pri.request_id  -- Fixed: was purchase_request_id
    WHERE NOT (ppr.project_id = ANY(proj_ids))
  );

  -- Delete purchase orders for purchase requests not in protected projects
  ...
  WHERE request_id IN (  -- Fixed: was purchase_request_id
    SELECT id FROM project_purchase_requests WHERE NOT (project_id = ANY(proj_ids))
  );
