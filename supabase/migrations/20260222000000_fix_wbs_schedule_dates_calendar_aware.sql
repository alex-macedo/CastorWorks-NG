-- Migration: Make initialize_wbs_schedule_dates calendar-aware
-- Purpose: When project has calendar_enabled=true, use working days for WBS schedule dates
--          so tasks no longer appear on weekends/holidays (fix for "Agendar Automaticamente").
-- Date: 2026-02-22
--
-- Changes:
-- - If calendar_enabled: use get_next_working_day and calculate_activity_end_date
-- - If not: keep existing calendar-day arithmetic (unchanged behavior)

BEGIN;

CREATE OR REPLACE FUNCTION public.initialize_wbs_schedule_dates(
  _project_id UUID
)
RETURNS TABLE(
  out_wbs_item_id UUID,
  out_start_date DATE,
  out_end_date DATE,
  out_duration_days INT
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_project_start_date DATE;
  v_calendar_enabled BOOLEAN;
  v_current_date DATE;
  v_item RECORD;
  v_parent_end_date DATE;
  v_child_duration INT;
  v_items_updated INT DEFAULT 0;
  v_phases_created INT DEFAULT 0;
  v_activities_updated INT DEFAULT 0;
BEGIN
  -- Get project start date and calendar setting
  SELECT p.start_date, COALESCE(p.calendar_enabled, false)
  INTO v_project_start_date, v_calendar_enabled
  FROM public.projects p
  WHERE p.id = _project_id;

  IF v_project_start_date IS NULL THEN
    RAISE EXCEPTION 'Project start_date is not set for project %', _project_id;
  END IF;

  -- If calendar enabled and project start is not a working day, move to next working day
  IF v_calendar_enabled AND NOT is_day_working(_project_id, v_project_start_date) THEN
    v_project_start_date := get_next_working_day(_project_id, v_project_start_date - 1);
  END IF;

  -- Create temp table to store calculated dates
  CREATE TEMP TABLE IF NOT EXISTS temp_wbs_dates (
    wbs_item_id UUID PRIMARY KEY,
    parent_id UUID,
    level INT,
    sort_order INT,
    start_date DATE,
    end_date DATE,
    duration_days INT,
    code_path TEXT,
    item_type public.wbs_item_type,
    item_name TEXT
  ) ON COMMIT DROP;

  TRUNCATE temp_wbs_dates;

  -- First pass: Calculate dates for all items using recursive CTE
  WITH RECURSIVE wbs_tree AS (
    SELECT 
      wi.id,
      wi.parent_id,
      wi.sort_order,
      wi.code_path,
      wi.standard_duration_days,
      wi.item_type,
      wi.name,
      1 AS level,
      ROW_NUMBER() OVER (ORDER BY wi.sort_order, wi.code_path) AS global_order
    FROM public.project_wbs_items wi
    WHERE wi.project_id = _project_id
      AND wi.parent_id IS NULL
    
    UNION ALL
    
    SELECT 
      wi.id,
      wi.parent_id,
      wi.sort_order,
      wi.code_path,
      wi.standard_duration_days,
      wi.item_type,
      wi.name,
      wt.level + 1,
      wt.global_order * 1000 + ROW_NUMBER() OVER (
        PARTITION BY wi.parent_id 
        ORDER BY wi.sort_order, wi.code_path
      )
    FROM public.project_wbs_items wi
    JOIN wbs_tree wt ON wt.id = wi.parent_id
    WHERE wi.project_id = _project_id
  )
  INSERT INTO temp_wbs_dates (wbs_item_id, parent_id, level, sort_order, duration_days, code_path, item_type, item_name)
  SELECT id, parent_id, level, sort_order, COALESCE(standard_duration_days, 1), code_path, item_type, name
  FROM wbs_tree
  ORDER BY global_order;

  -- Second pass: Calculate start dates sequentially (calendar-aware or not)
  v_current_date := v_project_start_date;
  
  FOR v_item IN (
    SELECT * FROM temp_wbs_dates 
    WHERE parent_id IS NULL 
    ORDER BY sort_order, code_path
  ) LOOP
    SELECT COALESCE(SUM(twd.duration_days), 0) INTO v_child_duration
    FROM temp_wbs_dates twd
    WHERE twd.parent_id = v_item.wbs_item_id;
    
    -- Phase end date: use working days if calendar enabled
    IF v_calendar_enabled THEN
      v_parent_end_date := calculate_activity_end_date(
        v_current_date,
        GREATEST(v_item.duration_days, v_child_duration, 1),
        _project_id,
        true
      );
    ELSE
      v_parent_end_date := v_current_date + GREATEST(v_item.duration_days, v_child_duration, 1) - 1;
    END IF;
    
    UPDATE temp_wbs_dates 
    SET start_date = v_current_date,
        end_date = v_parent_end_date,
        duration_days = GREATEST(v_item.duration_days, v_child_duration, 1)
    WHERE temp_wbs_dates.wbs_item_id = v_item.wbs_item_id;
    
    -- Process children of this phase
    DECLARE
      v_child RECORD;
      v_child_start DATE := v_current_date;
      v_child_end DATE;
    BEGIN
      FOR v_child IN (
        SELECT * FROM temp_wbs_dates 
        WHERE parent_id = v_item.wbs_item_id 
        ORDER BY sort_order, code_path
      ) LOOP
        IF v_calendar_enabled THEN
          v_child_end := calculate_activity_end_date(
            v_child_start,
            COALESCE(v_child.duration_days, 1),
            _project_id,
            true
          );
          UPDATE temp_wbs_dates 
          SET start_date = v_child_start,
              end_date = v_child_end
          WHERE temp_wbs_dates.wbs_item_id = v_child.wbs_item_id;
          v_child_start := get_next_working_day(_project_id, v_child_end);
        ELSE
          v_child_end := v_child_start + COALESCE(v_child.duration_days, 1) - 1;
          UPDATE temp_wbs_dates 
          SET start_date = v_child_start,
              end_date = v_child_end
          WHERE temp_wbs_dates.wbs_item_id = v_child.wbs_item_id;
          v_child_start := v_child_start + COALESCE(v_child.duration_days, 1);
        END IF;
      END LOOP;
    END;
    
    -- Next phase starts after this one ends
    IF v_calendar_enabled THEN
      v_current_date := get_next_working_day(_project_id, v_parent_end_date);
    ELSE
      v_current_date := v_parent_end_date + 1;
    END IF;
  END LOOP;

  -- Create missing project_phases records for phase-type WBS items
  INSERT INTO public.project_phases (
    project_id,
    phase_name,
    sort_order,
    status,
    progress_percentage,
    budget_allocated,
    budget_spent,
    wbs_item_id,
    type,
    start_date,
    end_date
  )
  SELECT
    _project_id,
    twd.item_name,
    COALESCE((SELECT wi2.sort_order FROM public.project_wbs_items wi2 WHERE wi2.id = twd.wbs_item_id), 0),
    'pending'::phase_status,
    0,
    0,
    0,
    twd.wbs_item_id,
    'schedule',
    twd.start_date,
    twd.end_date
  FROM temp_wbs_dates twd
  WHERE twd.item_type = 'phase'::public.wbs_item_type
    AND NOT EXISTS (
      SELECT 1 FROM public.project_phases pp
      WHERE pp.wbs_item_id = twd.wbs_item_id
        AND pp.project_id = _project_id
    )
  ON CONFLICT (project_id, wbs_item_id) WHERE wbs_item_id IS NOT NULL
  DO UPDATE SET
    start_date = EXCLUDED.start_date,
    end_date = EXCLUDED.end_date,
    type = EXCLUDED.type,
    updated_at = NOW();
  
  GET DIAGNOSTICS v_phases_created = ROW_COUNT;
  RAISE NOTICE 'Created/updated % project_phases records', v_phases_created;

  -- Update existing project_phases with calculated dates for phase-type WBS items
  FOR v_item IN (
    SELECT 
      twd.wbs_item_id,
      twd.start_date AS calc_start_date,
      twd.end_date AS calc_end_date
    FROM temp_wbs_dates twd
    WHERE twd.item_type = 'phase'::public.wbs_item_type
  ) LOOP
    UPDATE public.project_phases
    SET 
      start_date = v_item.calc_start_date,
      end_date = v_item.calc_end_date,
      updated_at = NOW()
    WHERE project_phases.wbs_item_id = v_item.wbs_item_id
      AND project_phases.project_id = _project_id;
  END LOOP;
  
  GET DIAGNOSTICS v_items_updated = ROW_COUNT;
  RAISE NOTICE 'Updated % project_phases with schedule dates', v_items_updated;

  -- Update project_activities with calculated dates for WBS-linked activities
  UPDATE public.project_activities pa
  SET start_date = twd.start_date,
      end_date = twd.end_date,
      days_for_activity = COALESCE(pa.days_for_activity, twd.duration_days),
      updated_at = NOW()
  FROM temp_wbs_dates twd
  WHERE pa.project_id = _project_id
    AND pa.wbs_item_id = twd.wbs_item_id;

  GET DIAGNOSTICS v_activities_updated = ROW_COUNT;
  RAISE NOTICE 'Updated % project_activities with schedule dates', v_activities_updated;

  -- Return the calculated dates with renamed output columns to avoid ambiguity
  RETURN QUERY
  SELECT 
    twd.wbs_item_id AS out_wbs_item_id,
    twd.start_date AS out_start_date,
    twd.end_date AS out_end_date,
    twd.duration_days AS out_duration_days
  FROM temp_wbs_dates twd
  ORDER BY twd.code_path;
END;
$$;

COMMENT ON FUNCTION public.initialize_wbs_schedule_dates(UUID) IS 
'Calculates and sets start/end dates for WBS items based on project start date and standard durations. 
When calendar_enabled=true, uses working days to avoid weekends/holidays.
Creates missing project_phases records if they don''t exist, then updates them with calculated schedule dates.
Phases are processed sequentially, with each phase starting after the previous one ends.
Children within a phase are also processed sequentially.';

-- Re-run WBS schedule for calendar-enabled projects to fix existing weekend dates
DO $$
DECLARE
  v_project_id UUID;
  v_row RECORD;
BEGIN
  FOR v_project_id IN
    SELECT id FROM public.projects WHERE COALESCE(calendar_enabled, false) = true
  LOOP
    PERFORM * FROM public.initialize_wbs_schedule_dates(v_project_id);
    RAISE NOTICE 'Re-initialized WBS schedule for calendar project %', v_project_id;
  END LOOP;
END;
$$;

COMMIT;
