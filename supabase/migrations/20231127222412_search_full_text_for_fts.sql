...
alter table page
add column if not exists fts_tokens tsvector generated always as (to_tsvector('english', content)) stored;

create index if not exists fts_search_index_page on page using gin(fts_tokens);
...
add column if not exists title_tokens tsvector generated always as (to_tsvector('english', coalesce(meta ->> 'title', ''))) stored;

create index if not exists fts_search_index_title on page using gin(title_tokens);
